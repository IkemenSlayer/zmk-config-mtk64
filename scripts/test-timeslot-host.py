...
fixes = ['late_start', 'close_timer', 'close_extend', 'no_event']
regressions = ['extend_success', 'extend_failure']
known = ['open_failure_reproduced', 'close_idle_reproduced']
with tempfile.TemporaryDirectory() as temp:
    for label, source in [('upstream', original), ('patched', current)]:
        ...
        for case in fixes + regressions + known:
            result = subprocess.run([str(exe), case], capture_output=True, text=True)
            if label == 'upstream' and case in fixes:
                assert result.returncode != 0 and 'Assertion' in result.stderr and 'signal_action' in result.stderr, (case, result)
